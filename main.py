...
response = ""
prompt = ""
big_context = """You are an AI assistant designed to provide users with hobbies / volunteering opportunities / productivity tips / advice.
        They have provided some of their life information to help you give advice.
...
def hobbies():   
    if request.method == 'POST':
        ...
        Always end your response with a complete sentence."""
        global big_context
        big_context = big_context  + """Their daily schedule: """ + response1 + """. Are they physically active: """ + response2 + """ 
        Do they enjoy social interaction: """ + response3 + """ Which types of activities they prefer: """ + response4 + """ 
        What they spend their time doing: """ + response5 + """ Mind type: """ + response6 + """ 
        Favorite school subject: """ + response7 + """ Content watched: """ + response8 + """ 
        Specific Interests: """ + response9 + """ Current hobbies: """ + response12 + """ 
...
def productivity():   
    if request.method == 'POST':
        ...
        score = generate("Give the user a productivity score on a scale of 1 to 100. Format it like this: Productivity Score: 'score given'.", context)
        return render_template('output.html', previous_text = output)
    if request.method == 'GET':
...
def volunteering():
    if request.method == 'POST':
        ...
        Always end your response with a complete sentence."""
        global big_context
        big_context = big_context  + "What they are passionate about: """ + response1 + """
         Favorite School Subject: """ + response2 + """
         Activities they participate in: """ + response3 + """
         Desired level of activity during volunteering""" + response4 + """
         Do they want to be hands-on when volunteering""" + response5 + """
         ...
         Their favorite activities: """ + response8 + """
         Time they desire to spend volunteering: """ + response9 + """
         Reason for volunteering: """ + response10
...
def chat():
    if request.method == 'POST': 
        ...
        response = generate(prompt, big_context)
        
        return render_template('chat.html', processed_text=response)
    if request.method == 'GET':
